...
class MG6010:
    ...
    def __init__(self, motor_id: int = 1, channel: str = "can0", bustype: str = "socketcan", response_timeout: float = 0.2, reduction: int = 6):
    ...
    def _flush_rx(self) -> None:
        """Flush stale messages from CAN receive buffer."""
        if self.bus is not None:
            try:
    ...
    def _send_recv(self, data: bytes, retries: int = 1) -> bytes:
        if self.bus is None:
    ...
    def read_position(self) -> float:
        data = bytes([self.CMD_READ_POSITION]) + b"\x00" * 7
    ...
    def set_zero_position(self) -> None:
        """Escribe en la ROM el punto cero actual. El motor no envía respuesta a este comando."""
        if self.bus is None:
            raise MG6010Error("Not connected")
